...
# The pairing of src and class is used by intellij to attatch sources.
#
...
#   * exports should only accept JavaInfo's (this include KotlinInfo) as the jars attribute takes care of importing the jars without generating ijars.
...
Args:
  jars: The jars listed here are exported as kotlin libraries Either a single class jar, or labels to filegroups. When the labels is a file_provider it should
    follow the conventions used in repositories generated by the maven_jar rule  --i.e., the rule expects a file_provider with a single class jar and a
    single source jar. a source jar is recognized by the suffix `-sources.jar`.
