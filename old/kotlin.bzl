load("old/old_kotlin_repositories.bzl", "kotlin_repositories")
load("//kotlin/internal:defs.bzl", "REPO_ROOT")
...
def _kotlin_compile_impl(ctx):
    ...
    ctx.actions.run(
        ...
        env = {
            "KOTLIN_COMPILER": ctx.file._kotlin_compiler.path
        }
    )
...
_kotlin_compile_attrs = {
    ...
    "_kotlinw": attr.label(
        default=Label("//kotlin/compiler/jvm:worker"),
        executable = True,
        cfg = 'host',
    ),
